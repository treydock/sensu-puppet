...
Puppet::Type.type(:sensu_agent_entity_config).provide(:sensu_api, :parent => Puppet::Provider::SensuAPI) do
  ...
  def exists?
    return true if (self.class.agent_managed_entity != nil && self.class.agent_managed_entity.to_sym == :true)
    return true if (@property_hash[:agent_managed_entity] != nil && @property_hash[:agent_managed_entity].to_sym == :true)
